...
class ServiceV2OperabilityTests(unittest.TestCase):
    def test_python_ci_installs_evidence_test_dependencies(self) -> None:
        ...
        python_job = workflow.split("  python-integration:", 1)[1].split("  postgres-journal:", 1)[0]
        self.assertIn("requirements-release.txt", python_job)
        self.assertIn("python -m pytest python/tests -q", python_job)
    ...
    def test_exact_candidate_pytest_gate_is_pinned_and_retains_failures(self) -> None:
        ...
        release = (REPO_ROOT / ".github" / "workflows" / "release-readiness.yml").read_text(encoding="utf-8")
        self.assertIn("Project already-proven exact-SHA quality gates", reusable)
        self.assertIn("project-gate-evidence", reusable)
        self.assertNotIn("cargo test --workspace", reusable)
        self.assertLess(release.index("Qualification preflight"), release.index("Protected release approval"))
        self.assertIn("Run synthetic assemble and verifier contract tests", release)
