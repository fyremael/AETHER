use aether_ast::{
    ...
    OperationKind, PredicateId, PredicateRef, ReplicaId, RuleAst, RuleId, RuleProgram, Term, Value,
    Variable,
};
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    println!("===========================================");
    println!(
        "One compiled recursive program, two semantic snapshots, and a proof trail you can inspect."
    );
    println!();
...
fn print_snapshot(
    ...
    let trace =
        InMemoryExplainer::from_derived_set(derived).explain_tuple(&deepest_tuple.tuple.id)?;
...
fn print_trace(trace: &DerivationTrace) {
    for tuple in &trace.tuples {
        ...
        println!(
            "    t{}: {} -> {} | rule {} | iteration {} | parents {} | sources {}",
            tuple.tuple.id,
            entity_label(*from),
            entity_label(*to),
            tuple.metadata.rule_id,
            tuple.metadata.iteration,
            render_tuple_ids(&tuple.metadata.parent_tuple_ids),
            render_elements(&tuple.metadata.source_datom_ids)
        );
    }
}
...
fn render_tuple_ids(tuple_ids: &[aether_ast::TupleId]) -> String {
    if tuple_ids.is_empty() {
        return "[]".into();
    }

    let rendered = tuple_ids
        .iter()
...
fn render_elements(elements: &[ElementId]) -> String {
